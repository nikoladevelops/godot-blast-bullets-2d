...
func _test_circle_layers() -> void:
	...
	var base: Array = BulletFactory2D.helper_generate_transforms_circle(12, marker, 150.0, true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	var layered: Array = BulletFactory2D.helper_generate_transforms_circle(12, marker, 150.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
...
func _test_ring_layers() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	var base: Array = BulletFactory2D.helper_generate_transforms_ring(12, marker, 150.0, 0.0, TAU, true, false, true, 1.0, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	var layered: Array = BulletFactory2D.helper_generate_transforms_ring(12, marker, 150.0, 0.0, TAU, true, false, true, 1.0, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
...
func _test_rect_layers() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	var base: Array = BulletFactory2D.helper_generate_transforms_rectangle(8, marker, Vector2(300, 200), true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	var layered: Array = BulletFactory2D.helper_generate_transforms_rectangle(8, marker, Vector2(300, 200), true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
...
func _test_polygon_layers() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	var base: Array = BulletFactory2D.helper_generate_transforms_polygon(12, marker, 6, 150.0, 0.0, true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	var layered: Array = BulletFactory2D.helper_generate_transforms_polygon(12, marker, 6, 150.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
	_check_pair(base, layered, marker.origin, 3, STEP, 0, "polygon")
	var sq_base: Array = BulletFactory2D.helper_generate_transforms_rectangle(4, marker, Vector2(200, 200), true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	_check(_approx((sq_base[0] as Transform2D).get_rotation(), -PI * 0.5, 0.01), "square bullet 0 faces straight")
	_check(BulletFactory2D.helper_generate_transforms_triangle(9, marker, 0, 200.0, 200.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0).size() == 9, "triangle emits in LAYERS")
	_check(BulletFactory2D.helper_generate_transforms_trapezoid(8, marker, 150.0, 280.0, 140.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0).size() == 8, "trapezoid emits in LAYERS")
	_check(BulletFactory2D.helper_generate_transforms_diamond(8, marker, 280.0, 200.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0).size() == 8, "diamond emits in LAYERS")

func _test_sequential_fill() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	# Sequential fills layer 0 first: the first chunk rides the base circle.
	var layered: Array = BulletFactory2D.helper_generate_transforms_circle(12, marker, 150.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 1, 0)
...
func _test_inward_and_both() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	var inward: Array = BulletFactory2D.helper_generate_transforms_circle(9, marker, 150.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 1, 0, 0)
	var worst := 0.0
	...
	var both: Array = BulletFactory2D.helper_generate_transforms_circle(9, marker, 150.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 2, 0, 0)
	worst = 0.0
	for i in both.size():
...
func _test_star_heart_emit() -> void:
	var marker := Transform2D(0.0, Vector2(400, 300))
	var base_flower: Array = BulletFactory2D.helper_generate_transforms_flower(24, marker, 6, 5, 150.0, 0.5, 1.0, 0.0, true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0, 0, 0.0, 45.0, 80.0, 6.0, 1.0)
	var layered_flower: Array = BulletFactory2D.helper_generate_transforms_flower(24, marker, 6, 5, 150.0, 0.5, 1.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0, 0, 0.0, 45.0, 80.0, 6.0, 1.0)
	_check_pair(base_flower, layered_flower, marker.origin, 3, STEP, 0, "flower")
	var star: Array = BulletFactory2D.helper_generate_transforms_star(20, marker, 5, 150.0, 65.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
	_check(star.size() == 20, "star emits in LAYERS")
	var base_rose: Array = BulletFactory2D.helper_generate_transforms_rose(24, marker, 6, 150.0, 1.0, 0.0, true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	var layered_rose: Array = BulletFactory2D.helper_generate_transforms_rose(24, marker, 6, 150.0, 1.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
	...
	var heart: Array = BulletFactory2D.helper_generate_transforms_heart(40, marker, 150.0, 0.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 3, STEP, 0, 0, 0)
	_check(heart.size() == 40, "heart emits in LAYERS (was missing)")
	var base_heart: Array = BulletFactory2D.helper_generate_transforms_heart(40, marker, 150.0, 0.0, true, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
...
func _test_translation_and_collapse() -> void:
	...
	var marker := Transform2D(0.0, Vector2(400, 300))
	var ring: Array = BulletFactory2D.helper_generate_transforms_ring(4, marker, 100.0, 0.0, TAU, true, false, true, 1.0, 0.0, 0, 0, false, 0, 32.0, false, 0.0, 1, STEP, 0, 0, 0)
	...
	var flat: Array = BulletFactory2D.helper_generate_transforms_circle(12, marker, 150.0, true, 0.0, 1, 0, false, 0, 32.0, false, 0.0, 64, 8.0, 1, 0, 0)
