...
func _on_area_entered(hit_target_area: Object, _multimesh_bullets_instance:MultiMeshBullets2D, _bullet_index:int, shared_bullets_custom_data: Resource, _bullet_global_transform: Transform2D) -> void:	
	if hit_target_area is AbstractEnemy:
		...
		if dmg_data.is_player_owned == false: # If it wasn't the player who spawned the bullet, then that means an enemy is hitting another enemy - I want the bullet to dissapear without it damaging the enemy (No friendly fire :P)
			return
		hit_target_area.take_damage(dmg_data.base_damage) # You can do way more complex damage logic with the rest of the properties inside shared_bullets_custom_data, you can do anything..
...
func _on_body_entered(hit_target_body: Object, _multimesh_bullets_instance:MultiMeshBullets2D, _bullet_index:int, shared_bullets_custom_data: Resource, _bullet_global_transform: Transform2D) -> void:
	if hit_target_body is Player:
		var dmg_data:DamageData = shared_bullets_custom_data as DamageData
		hit_target_body.take_damage(dmg_data.base_damage)
		
	#print("Bullet just collided with a body")

func _on_life_time_over(_multimesh_bullets_instance: MultiMeshBullets2D, _bullet_indexes:Array[int], _shared_bullets_custom_data: Resource, _bullets_global_transforms: Array[Transform2D]) -> void:
	pass
