...
#### If you want aimed patterns without writing any math, use `BulletSpawner2D`<br>

`BulletSpawner2D` is a `Node2D` that owns the whole firing loop: it builds one global `Transform2D` per bullet from `transforms_source`, then spawns through the factory. Point `bullet_factory_path` at your factory, assign `spawn_data`, and either call `shoot_once()` yourself or turn on `shooting_enabled` for the timed auto loop (`shoot_interval_sec`, `shoot_initial_delay_sec`, `max_volleys`, `reload_jitter_sec`).

- **24 pattern sources** - `From Children` (one transform per `Node2D` child marker), `From Self` (single bullet), plus 22 procedural helpers, each with its own `helper_*` inspector group: `Grid`, `Ring`, `Fan` (with `helper_fan_angle_jitter` shotgun spread), `Spiral`, `Line`, `Aimed` (aims a cone at `helper_aimed_target`, with velocity prediction), `Flower`, `Ellipse` (ring / arc / wall-with-gaps), `Rain`, `Scatter`, `Polygon`, `MultiSpiral`, `Cross`, `Star`, `Heart`, `Wave`, `Waterfall`, `Lattice`, `Rose` (exact rhodonea bloom), `CounterSpiral` (twin counter-rotating arms), `Corridor` (dense wall with a carved center dodge door, steered by the aimed target when set), `Lissajous` (figure-8 weave). Pipeline per volley: generate, then spin, then `transforms_scale`, then `helper_skip_indices` carving (slot indexes dropped after spin/scale, so they match the editor preview - use them for extra dodge doors or bullet text).
- **Starter presets** - `apply_pattern_preset()` fills the `helper_*` properties and switches `transforms_source` for 19 one-call patterns (`PatternPreset`: radial dense/sparse, 3-arm spiral, narrow/wide aimed fan, slow ring, gap wall, rain, 6-petal flower, scatter/cross/star/heart/snake/waterfall, petal storm via Rose, twin counter-spiral, aimed trap via Corridor, blossom finale via Rose). Tune from there; the preset only writes properties, nothing fires.
- **Motion and phrasing** - `spin_enabled` (`SPIN_CONTINUOUS` at `spin_speed_deg_per_sec`, or `SPIN_OSCILLATE` with amplitude/frequency), `spawn_position_offset` muzzle nudge, `burst_enabled` / `burst_count` / `burst_interval_sec` / `burst_alternate_mirror`, and `telegraph_enabled` / `telegraph_sec` warnings (`volley_telegraphed` fires first, the real shot re-collects transforms so moving markers still aim right).
- **Sequencer** - `spawn_pattern_list(entries, simultaneous, interval_sec)` queues boss phases from Dictionaries with `preset` / `transforms_source` / `helper_bullets_amount` / `spawn_data` keys; sequential mode fires one entry per interval and emits `pattern_list_finished`, simultaneous mode fires all at once and returns the fired count.
- **Live preview** - `show_pattern_preview` draws one dot plus one facing arrow per bullet in the editor (follows the generator live, never saved to the scene); `show_preview_during_runtime` keeps it during play.
